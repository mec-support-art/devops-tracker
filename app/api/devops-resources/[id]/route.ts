...
export async function PATCH(
  ...
  try {
    ...
    await renamePersonReferences("devops_resources", currentName, name);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Unable to update DevOps resource.",
      },
      { status: 500 },
    );
  }
...
export async function DELETE(
  ...
  try {
    ...
    await ensurePersonIsUnused("devops_resources", existing.name);

    const { error } = await supabase.from("devops_resources").delete().eq("id", id);
    ...
    const message =
      error instanceof Error ? error.message : "Unable to delete DevOps resource.";
